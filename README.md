...
- `assets/liboke-de-poisson.jpg` (the fish-in-banana-leaf photo in the "What's a liboké?" section) is a Wikimedia Commons photo by user **Lucia236**, licensed **CC BY-SA 4.0** — free to use commercially, but the license requires visible attribution, which is why there's a credit + license link under the image (`#about` section in `index.html`). **Swap this for your own food photography once you have it** — a real shot of your own liboké will do more for the brand than a generic reference photo, and removes the attribution requirement entirely.
